...
public class Solution
{
  public int Candy(int[] ratings)
  {
    ...
    for (var i = 1; i < ratingsLength; i++)
    {
      ...
      candiesPerChildFromLeft[i] =
        ratings[i] > ratings[i - 1]
          ? candiesPerChildFromLeft[i - 1] + 1
          : candiesPerChildFromLeft[i];
    }
    ...
    for (var i = ratingsLength - 2; i >= 0; i--)
    {
      ...
      candiesPerChildFromRight[i] =
        ratings[i] > ratings[i + 1]
          ? candiesPerChildFromRight[i + 1] + 1
          : candiesPerChildFromRight[i];
    }
    ...
    {
      minCandies +=
        candiesPerChildFromRight[x] > candiesPerChildFromLeft[x]
          ? candiesPerChildFromRight[x]
          : candiesPerChildFromLeft[x];
    }
